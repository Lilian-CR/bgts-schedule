...
function within(now, s, e) {
  return now >= new Date(s) && now < new Date(e);
}
function past(now, e) {
  return now >= new Date(e);
}
...
function renderList() {
  ...
  $$('#schedule article.session.past').forEach((el) => {
    el.addEventListener('click', () => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#showAll').onclick = () =>
    $$('#schedule article').forEach((el) => (el.style.display = ''));
  $('#showCurrent').onclick = () => {
    const now = getNowMs();
    $$('#schedule article').forEach((el) => {
